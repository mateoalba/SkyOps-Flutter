...
class _AuditLogListScreenState extends State<AuditLogListScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<AuditLogProvider>().cargar();
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final items = provider.items.where((item) {
      ...
      return item.accion.toString().toLowerCase().contains(q) ||
          (item.descripcion ?? '').toString().toLowerCase().contains(q);
    }).toList();
    ...
    return Scaffold(
      ...
      floatingActionButton: puedeEscribir
          ? FloatingActionButton(
              heroTag: 'audit_log_nuevo_fab',
              onPressed: () => context.push('/audit-log/nuevo'),
      ...
      body: LoadingOverlay(
        ...
        child: RefreshIndicator(
          onRefresh: () => context.read<AuditLogProvider>().cargar(forzar: true),
          child: Column(
            children: [
              SearchField(
                ...
                child: items.isEmpty
                    ? ListView(
                        children: [
                          ...
                          Center(
                            child: Text(
                              provider.estado == EstadoCargaAuditLog.error
                    ...
                    : ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return EntityListCard(
                            titulo: item.accion.toString(),
                            subtitulo: item.descripcion ?? ''.toString(),
                            iniciales: item.accion.toString(),
                            activo: null,
                            onEliminar: esAdmin && item.id != null ? () => _eliminar(context, item.id!, item.accion.toString()) : null,
                            onTap: puedeEscribir ? () => context.push('/audit-log/${item.id}/editar') : null,
                          );
                        },
                      ),
              ),
