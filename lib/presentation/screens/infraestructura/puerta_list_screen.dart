...
class PuertaListScreen extends StatefulWidget {
  const PuertaListScreen({super.key});

  @override
  State<PuertaListScreen> createState() => _PuertaListScreenState();
}

class _PuertaListScreenState extends State<PuertaListScreen> {
  String _busqueda = '';
  ...
  Widget build(BuildContext context) {
    final provider = context.watch<PuertaProvider>();
    ...
    final items = provider.items.where((item) {
      ...
      return 'Puerta ${item.codigo}'.toString().toLowerCase().contains(q) ||
          item.terminal.toString().toLowerCase().contains(q);
    }).toList();
    ...
    return Scaffold(
      ...
      body: LoadingOverlay(
        ...
        child: RefreshIndicator(
          onRefresh: () => context.read<PuertaProvider>().cargar(forzar: true),
          child: Column(
            children: [
              SearchField(
                hintText: 'Buscar puerta...',
                ...
                child: items.isEmpty
                    ? ListView(
                        children: [
                          ...
                          Center(
                            child: Text(
                              provider.estado == EstadoCargaPuerta.error
                    ...
                    : ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return EntityListCard(
                            titulo: 'Puerta ${item.codigo}'.toString(),
                            subtitulo: item.terminal.toString(),
                            iniciales: 'Puerta ${item.codigo}'.toString(),
                            activo: null,
                            onEliminar: esAdmin && item.id != null ? () => _eliminar(context, item.id!, 'Puerta ${item.codigo}') : null,
                            onTap: () => context.push(esAdmin ? '/puertas/${item.id}/editar' : '/puertas/${item.id}'),
                          );
                        },
                      ),
              ),
