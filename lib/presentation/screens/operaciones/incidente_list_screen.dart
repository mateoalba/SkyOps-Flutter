...
class IncidenteListScreen extends StatefulWidget {
  const IncidenteListScreen({super.key});

  @override
  State<IncidenteListScreen> createState() => _IncidenteListScreenState();
}

class _IncidenteListScreenState extends State<IncidenteListScreen> {
  String _busqueda = '';
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<IncidenteProvider>().cargar();
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final items = provider.items.where((item) {
      ...
      return item.tipo.toString().toLowerCase().contains(q) ||
          item.severidad.toString().toLowerCase().contains(q);
    }).toList();
    ...
    return Scaffold(
      ...
      body: LoadingOverlay(
        ...
        child: RefreshIndicator(
          onRefresh: () => context.read<IncidenteProvider>().cargar(forzar: true),
          child: Column(
            children: [
              SearchField(
                ...
                child: items.isEmpty
                    ? ListView(
                        children: [
                          ...
                          Center(
                            child: Text(
                              provider.estado == EstadoCargaIncidente.error
                    ...
                    : ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return EntityListCard(
                            ...
                            subtitulo: item.severidad.toString(),
                            iniciales: item.tipo.toString(),
                            activo: null,
                            onEliminar: esAdmin && item.id != null ? () => _eliminar(context, item.id!, item.tipo) : null,
                            onTap: () => context.push(esAdmin ? '/incidentes/${item.id}/editar' : '/incidentes/${item.id}'),
                          );
                        },
                      ),
              ),
